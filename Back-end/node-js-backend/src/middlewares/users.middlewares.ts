...
import { checkSchema, header, ParamSchema } from "express-validator";
...
// 1 req của client  gữi lên sever sẽ có body (chứa các thông tin cần giử)
        ...
        Authorization: {
            ...
            custom: {
                options: async (value, { req }) => {
                    // value lúc này đại diện cho trường dữ liệu của nó || value là authorization: "Bearer access_toekn"
                ...
                custom: {
                    options: (value: string, { req }) => {
                        ...
                        if (!REGEX_USERNAME.test(value)) {
                            throw new Error(USERS_MESSAGES.USERNAME_IS_INVALID) //messages.ts thêm USERNAME_IS_INVALID: 'Username is invalid'
                        }
                        return true
                    }
                }
